...
# %% auto 0
__all__ = ['Perturber', 'LinfPerturber', 'L2Perturber', 'AdversarialExamplesAttack']
...
class L2Perturber(Perturber):
    "Perturbs inputs under a bounded $l_2$ norm"
    @classmethod
    def rand_init(cls, epsilon, shape):
        rand_dir = _random_unit_vector(shape)
        rand_len = torch.rand(1) * epsilon
    ...
    def callback(self):
        class ProjectionCallback(Callback):
            ...
            def after_batch(inner_self):
                with torch.no_grad():
                    self.p.mul_(torch.min(self.epsilon/_batch_norm(self.p), torch.ones_like(self.p)))
                    self.clamp_pixel_values(inner_self.x)

        return ProjectionCallback()


# %% ../nbs/evasion.ipynb 10
